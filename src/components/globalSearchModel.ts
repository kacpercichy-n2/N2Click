...
/**
 * Trasy spoza `NAV`, które paleta ma jednak proponować: Zgłoszenia żyją jako
 * przypięty link w stopce sidebara, a Struktura zespołu jako zakładka obszaru
 * Zespół — obie są pełnoprawnymi trasami bez własnego wpisu w menu.
 */
const EXTRA_NAV_TARGETS: Array<[string, string]> = [
  ['/team', 'Struktura zespołu'],
  ['/zgloszenia', 'Zgłoszenia'],
];

/** Dodatkowe hasła wyszukiwania per trasa (synonimy historycznych etykiet). */
const NAV_EXTRA_KEYWORDS: Record<string, string[]> = {
  '/admin': ['administracja'],
};
...
export function quickActionCatalog(opts: {
  ...
  const navigation: QuickAction[] = targets
    ...
    .map(([path, label]) => ({
      ...
      hint: 'Nawigacja',
      keywords: [label, path, 'przejdz', 'otworz widok', ...(NAV_EXTRA_KEYWORDS[path] ?? [])],
