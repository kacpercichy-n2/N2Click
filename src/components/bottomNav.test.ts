...
const DEFAULT_PATHS = NAV_ITEMS.map(([to]) => to);
const LABELS = new Map(NAV_ITEMS.map(([to, label]) => [to, label]));
...
describe('moreNavPaths', () => {
  ...
  it('lista po bramkach uprawnień nie odzyskuje ukrytych tras', () => {
    const gated = orderNavPaths(DEFAULT_PATHS, undefined).filter(
      (p) => p !== '/admin' && p !== '/team',
    );
